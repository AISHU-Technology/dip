 ...
 */
const createAdTreeNodeData = (
  dataSource: any[],
  options: CreateTreeDataOptions = {},
): AdTreeDataNode[] => {
  const newOptions = {
    titleField: 'name',
    keyField: 'key',
  ...
  const loop = (data: any[], parentKey: string, keyPath: string[]): AdTreeDataNode[] => {
    return data.map((item: any) => {
      const nodeKey =
        typeof newOptions.keyField === 'string'
          ? item[newOptions.keyField]
          : newOptions.keyField(item)
      const isLeaf =
        typeof newOptions.isLeaf === 'boolean' ? newOptions.isLeaf : newOptions.isLeaf?.(item)
      ...
      return {
        ...
        children:
          item.children && item.children.length > 0
            ? loop(item.children, item.key, newKeyPath)
            : [],
        sourceData: {
          ...item,
        },
...
const deleteTreeNode = (treeData: AdTreeDataNode[], deleteNodeKey: string | string[]) => {
  ...
  const newFlatTreeData = flatTreeData(treeData).filter(
    (item) => !deleteKeys.includes(item.key as string),
  )
  return flatToTreeData(newFlatTreeData)
}
...
const updateTreeNode = (
  treeData: AdTreeDataNode[],
  updateNode: AdTreeDataNode | AdTreeDataNode[],
) => {
